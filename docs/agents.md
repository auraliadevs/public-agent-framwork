...
## Implementation in Auralia Agent Framework
...
Our Auralia Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new AuraliaAgent({
  llm: new OllamaChatLLM(), // for more explore 'Auralia-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Auralia-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Auralia-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Auralia_advanced.ts) to see how to modify them.
